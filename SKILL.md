...
1. **Ask the user to confirm the output folder name.**
   ...
   - Example: If repos are `funds-reservation-settlement-poc` and `clearbank-adapter`, suggest `funds-reservation-settlement` or similar.
...
- **Business Rules Catalogue** — split by rule domain (e.g. financial rules, lifecycle rules, permission rules)
...
9. **Cascade updates (Mandatory — Non-Negotiable)**
   ...
   - **Trigger condition:** The cascade check is triggered whenever user feedback introduces ANY of the following:
     1. A new fact not previously documented (e.g., "zero-value settlement is always created").
     2. A contradiction of an existing fact (e.g., "EoD job also unlocks lodgement and workspace" when FUNC-017 said it only cleared FSS flags).
     3. A change to who/what/when/how of a behaviour (e.g., "only Buyer Rep can set completion date" when FUNC-007 said "Conveyancer").
     4. A new relationship or dependency (e.g., "rollover to next slot same day before EoD unlock").
...
```json
{
  "workflowVersion": "5.0",
  "status": "in-progress",
  "outputFolder": "funds-reservation-settlement",
  "repos": [
    {"name": "funds-reservation-settlement-poc", "path": "boe-funds-reservation/funds-reservation-settlement-poc", "addedDate": "2026-07-01"}
  ],
...
3. **Update .progress.json** — Set the artefact status to `"partial"`:
   ```json
   "07": {"status": "partial", "completedSections": ["financial-rules", "lifecycle-rules"], "remainingSections": ["permission-rules", "timing-rules"]}
   ```
