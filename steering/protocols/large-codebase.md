...
1. **Identify domains** from folder structure and naming (e.g. `settlement/`, `reservation/`, `approval/`)
...
```
Domain: Settlement
  Key files: settlement-service.kt, settlement-repository.kt, settlement-events.kt
  Business rules found: 5 (BR-001 to BR-005)
  Entities found: 3 (SettlementWorkspace, SettlementTransaction, Participant)
  Integrations found: 2 (ClearBank, LandRegistry)
  Open questions: 2

Domain: Funds Reservation
  ...
  Integrations found: 1 (ClearBank)
  Open questions: 1
```
...
- Provide it with a focused question (e.g. "What business rules exist in the settlement-service?")
...
3. Record in `.progress.json`:
   ...
   "domains": ["settlement", "reservation", "approval"]
   ```
